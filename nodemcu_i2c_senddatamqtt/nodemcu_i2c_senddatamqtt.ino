...
const char* ssid = "Teknisi";//put your own wifi ssid here
const char* password = "bondowoso";// put your wifi password here
...
void callback(char* topic, byte* payload, unsigned int length) 
{
  ...
  for(int i=0;i<length;i++)
  {
    ...
    lcd.setCursor(0, 0);
    lcd.print("FATONI LCD"); // Start Print text to Line 1
    lcd.setCursor(i, 1);
  ...
  while (!client.connected()) 
  {
    ...
    if (client.connect(clientId.c_str()))
    {
     ...
     //once connected to MQTT broker, subscribe command if any
      client.subscribe("FatoniCommand");
    } else {

...
byte startMinute = 14;  // Don't add leading zero to hour or minute - 7, not 07
byte endHour = 17;      // Use 24h format for the hour, without leading zero
byte endMinute = 15;

//waktu 2
byte startHour2 = 17;    // Set our start and end times
byte startMinute2 = 16;  // Don't add leading zero to hour or minute - 7, not 07
...
byte checkStartTime() {
  ...
  if ((now.hour() == startHour && now.minute() == startMinute)||(now.hour() == startHour2 && now.minute() == startMinute2)) {
...
byte checkEndTime() {
  ...
  if ((now.hour() == endHour && now.minute() == endMinute)||(now.hour() == endHour2 && now.minute() == endMinute2)) {

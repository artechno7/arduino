...
int yy = 0;
String Day = "  ";
uint8_t StartHH  = 21, StartMM  = 43, FinishHH  = 21, FinishMM  = 44, setMode = 0, setAlarm = 0, alarmMode=1; //alarmmode 0 defaulnya 0, 1 agar alarm langsung on
...
//reset eeprom dengan nilai semua rep
int rep = 0;
...
void setup()
{
 ...
 pinMode(9,OUTPUT);digitalWrite (9,OFF); 
  ...
  lcd.setCursor (0,1);
  lcd.print(F(" 8 Channel Relay "));
  delay (2000);
  lcd.clear();
//WriteEepromReset (); //mereset niali eeprom biar gak 255 comment hanya mau reset saja
ReadEeprom();
//Set RTC Untuk Pertama kali
//rtc.setDOW(WEDNESDAY);
//rtc.setTime (16, 42, 00); 
...
void setupClock (void) {
   ...
   btnALR.read();
    ...
    lcd.setCursor (0,1);
    lcd.print (F("  Date Saved "));delay (1000);
...
void WriteEepromReset () {
  EEPROM.write(1,rep); //(x,y) x= urutan eeprom y=data
  EEPROM.write(2,rep);
  EEPROM.write(3,rep);
  EEPROM.write(4,rep);
